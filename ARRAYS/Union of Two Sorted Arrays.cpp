...
//same approach with some extra conditions : duplicate elements are present in both the arrays.
//NAIVE APPROACH : can be to use set
//code 1:
vector<int> findUnion(int arr1[], int arr2[], int n, int m)
    {
        ...
        while(s1<n && s2<m){
            if(arr1[s1]==arr2[s2]) {
                if((ans.size()==0)||(ans.size()>0 &&ans.back()!=arr1[s1])) ans.push_back(arr1[s1]);
                s1++;
                s2++;
            }
               ...
               if( (ans.size()==0) ||(ans.size()>0 && ans.back()!=arr2[s2])) ans.push_back(arr2[s2]);
                s2++; 
            }
        }
        while(s1<n){
            if((ans.size()>0 &&ans.back()!=arr1[s1]))ans.push_back(arr1[s1]);
            s1++;
        }
         while(s2<m){
            if((ans.size()>0 &&ans.back()!=arr2[s2]))ans.push_back(arr2[s2]);
            s2++;
        }
        return ans;
    }
